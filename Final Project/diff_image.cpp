...
{
    string name_serial(argv[1]);
    string name_parallel(argv[2]);

    ifstream f1;
    ifstream f2;
    
    f1.open(name_serial);
    f2.open(name_parallel);

    if (!f1.is_open())
    {
        cout << name_serial << " open failed! \n";
        return 1;
    }
    if (!f2.is_open())
    {
        cout << name_parallel << " open failed! \n";
        return 1;
    }
    ...
    serial_img = my_reader -> ReadBMP(name_serial.c_str(), &img_col, &img_row);
    cuda_img = my_reader -> ReadBMP(name_parallel.c_str(), &img_col, &img_row);
    ...
    printf("Serial image, check difference in %s and %s with size of W: %d. H: %d \
            \nGot %d diffs in %d points rate %3.1f percent \n", name_serial.c_str(), name_parallel.c_str(), img_col, img_row, diff_cnt, img_size, (float) diff_cnt * 100.0f / (float) img_size);
    return 0;
}
