...
__global__ void cuda_gaussian_filter(unsigned char* cuda_input_image, unsigned char* cuda_output_image,int img_col, int img_row, int shift, unsigned int* cuda_filter_G, int filter_row, unsigned int filter_scale, int img_border)
{
    ...
    for(int j = 0; j < filter_row; j++)
    {
        for(int i = 0; i < filter_row; i++)
        {
			...
			tmp += cuda_filter_G[j * filter_row + i] * cuda_input_image[target];  
        }
    }
    tmp /= filter_scale;
    ...
    cuda_output_image[3 * (cuda_row * img_col + cuda_col) + shift] = tmp;
...
{
    bmpReader->WriteBMP(outputblur_name.c_str(), img_col, img_row, output_image);
...
int main(int argc, char* argv[])
{
    ...
    const dim3 grid_size((img_col + 32 - 1) / 32, (img_row + 32 - 1) / 32);
    for(int i = 0; i < 3; i++) //R G B channel respectively
    {
        cuda_gaussian_filter<<<grid_size, block_size>>>(cuda_input_image, cuda_output_image, img_col, img_row, i, cuda_filter_G, filter_row, filter_scale, resolution);
